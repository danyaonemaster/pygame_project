import pygame

import pygame_project.config as config
from pygame_project.assets.hp import Health
from pygame_project.config import (
    STATE_IDLE, STATE_JUMP, STATE_RUN, STATE_FALL,
    STATE_ATK2, STATE_ATK_AIR, STATE_ATK_RUN, STATE_ATK1,
    MOVE_DIRECTION_RIGHT, MOVE_DIRECTION_LEFT
)


class Player(pygame.sprite.Sprite):
    """
    Player character class.

    Responsibilities:
    - Handle player movement (left, right, jump, gravity)
    - Handle collisions with tiles using masks
    - Manage animation states and transitions
    - Handle attacks and combo system
    - Handle damage, invulnerability, and death
    """

    def __init__(self, xy_pairs, scale_size):
        super().__init__()

        # ---------------- STATE & ANIMATIONS ----------------
        self.state = STATE_IDLE
        self.animations = {}

        # Load all animations from config
        for name, paths in config.K_ANIMATIONS.items():
            frames = self.load_animation(paths, scale_size)
            self.animations[name] = {
                MOVE_DIRECTION_RIGHT: frames,
                MOVE_DIRECTION_LEFT: [pygame.transform.flip(f, True, False) for f in frames]
            }

        # ---------------- ANIMATION CONTROL ----------------
        self.frame = 0
        self.animation_speed = 0.15
        self.image = self.animations[self.state][MOVE_DIRECTION_RIGHT][self.frame]

        # ---------------- DAMAGE / INVULNERABILITY ----------------
        self.invulnerable = False
        self.invuln_time = 1000  # milliseconds

        # ---------------- COLLISION DATA ----------------
        self.rect = self.image.get_rect(midbottom=xy_pairs)
        self.mask = pygame.mask.from_surface(self.image)

        # ---------------- PHYSICS ----------------
        self.health = Health(20)
        self.gravity = 0
        self.on_ground = True
        self.ground_y = None

        # ---------------- COMBAT ----------------
        self.attack_stage = 0
        self.combo_queued = False
        self.attacking = False

        # ---------------- MOVEMENT STATE ----------------
        self.jumped = False
        self.moves = False
        self.side = 1  # 1 = right, 0 = left
        self.last_move_time = 0

        # ---------------- MOVEMENT VALUES ----------------
        self.move_speed = config.WIDTH * 0.005
        self.jump_strength = -config.HEIGHT * 0.020
        self.gravity_speed = config.HEIGHT * 0.0009375

    # --------------------------------------------------
    # Load animation frames and scale them
    # --------------------------------------------------
    def load_animation(self, paths, scale):
        frames = []
        for path in paths:
            image = pygame.image.load(path).convert_alpha()
            image = pygame.transform.scale(image, scale)
            frames.append(image)
        return frames

    # --------------------------------------------------
    # Change animation state safely
    # --------------------------------------------------
    def set_state(self, new_state):
        if self.state != new_state:
            self.state = new_state
            self.frame = 0

    # --------------------------------------------------
    # Apply gravity and resolve vertical collisions
    # --------------------------------------------------
    def apply_gravity(self, tiles):
        # Apply gravity force
        self.gravity += self.gravity_speed
        self.rect.y += self.gravity

        self.on_ground = False

        # Check collision with each tile using masks
        for tile in tiles:
            offset = (
                tile.rect.x - self.rect.x,
                tile.rect.y - self.rect.y
            )

            if self.mask.overlap(tile.mask, offset):
                # Landing on top of a tile
                if self.gravity > 0:
                    self.rect.bottom = tile.rect.top
                    self.gravity = 0
                    self.on_ground = True
                    self.jumped = False
                    self.ground_y = self.rect.bottom
                    return

        # Lock player to last ground Y (prevents jitter)
        if self.on_ground and self.ground_y is not None:
            self.rect.bottom = self.ground_y

    # --------------------------------------------------
    # Handle keyboard movement input
    # --------------------------------------------------
    def handle_input(self, current_time):
        keys = pygame.key.get_pressed()

        # Disable movement while attacking
        if self.attacking:
            return

        # Move right
        if keys[pygame.K_d]:
            self.rect.x += self.move_speed
            self.set_state(STATE_RUN)
            self.moves = True
            self.side = 1
            self.animate_run(self.animations[self.state][MOVE_DIRECTION_RIGHT])
            self.last_move_time = current_time

        # Move left
        elif keys[pygame.K_a]:
            self.rect.x -= self.move_speed
            self.set_state(STATE_RUN)
            self.moves = True
            self.side = 0
            self.animate_run(self.animations[self.state][MOVE_DIRECTION_LEFT])
            self.last_move_time = current_time

        # Jump / fall animations
        if self.jumped:
            if self.gravity < 0:
                self.set_state(STATE_JUMP)
            else:
                self.set_state(STATE_FALL)
            self.animate(self.animations[self.state])

        # Jump input
        if keys[pygame.K_SPACE] and self.on_ground:
            self.gravity = self.jump_strength
            self.on_ground = False
            self.jumped = True
            self.ground_y = None

        # Stop running animation after delay
        if current_time - self.last_move_time > 200:
            self.moves = False
            self.last_move_time = 0

    # --------------------------------------------------
    # Handle attack input logic
    # --------------------------------------------------
    def handle_attack_input(self):
        if self.attack_stage == 0:
            self.start_attack(1)
        elif self.attack_stage == 1:
            self.combo_queued = True

    # --------------------------------------------------
    # Generic animation update
    # --------------------------------------------------
    def animate(self, anim, slow_index=0.0):
        self.frame += self.animation_speed - slow_index
        if self.frame >= len(anim[MOVE_DIRECTION_RIGHT]):
            self.frame = 0

        self.image = anim[self.side][int(self.frame)]
        self.mask = pygame.mask.from_surface(self.image)

    # --------------------------------------------------
    # Run animation (horizontal movement)
    # --------------------------------------------------
    def animate_run(self, anim):
        self.frame += self.animation_speed
        if self.frame >= len(anim):
            self.frame = 0

        self.image = anim[int(self.frame)]
        self.mask = pygame.mask.from_surface(self.image)

    # --------------------------------------------------
    # Start an attack
    # --------------------------------------------------
    def start_attack(self, stage):
        self.attacking = True
        self.attack_stage = stage
        self.frame = 0

        if self.jumped:
            self.set_state(STATE_ATK_AIR)
        elif self.moves:
            self.set_state(STATE_ATK_RUN)
        else:
            self.set_state(STATE_ATK1)

        if stage == 2:
            self.set_state(STATE_ATK2)

    # --------------------------------------------------
    # End attack and reset state
    # --------------------------------------------------
    def end_attack(self):
        self.attacking = False
        self.attack_stage = 0
        self.combo_queued = False
        self.set_state(STATE_IDLE)

    # --------------------------------------------------
    # Animate attack and handle combos
    # --------------------------------------------------
    def animate_attack(self):
        anim = self.animations[self.state]
        self.frame += self.animation_speed

        combo_window = len(anim[MOVE_DIRECTION_RIGHT]) * 0.7

        if self.attack_stage == 1 and self.frame >= combo_window:
            if self.combo_queued:
                self.combo_queued = False
                self.start_attack(2)
                return

        if self.frame >= len(anim[MOVE_DIRECTION_RIGHT]):
            self.end_attack()
            return

        self.image = anim[self.side][int(self.frame)]
        self.mask = pygame.mask.from_surface(self.image)

    # --------------------------------------------------
    # Damage handling
    # --------------------------------------------------
    def take_damage(self, amount, current_time, pos):
        if self.invulnerable:
            return False

        self.health.damage(amount, current_time)
        self.invulnerable = True

        if self.health.is_dead():
            self.die(pos)
            return True

        return False

    # --------------------------------------------------
    # Update method (called every frame)
    # --------------------------------------------------
    def update(self, event_list, current_time, target_tile):
        self.handle_input(current_time)
        self.apply_gravity(target_tile)

        for event in event_list:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_x:
                    self.handle_attack_input()

        # Invulnerability timer
        if self.invulnerable:
            if current_time - self.health.last_hit >= self.invuln_time:
                self.invulnerable = False

        # Attack animation has priority
        if self.attacking:
            self.animate_attack()
            return

        # Idle state
        if not self.moves and not self.jumped and not self.attacking:
            self.set_state(STATE_IDLE)
            self.animate(self.animations[self.state], 0.1)

    # --------------------------------------------------
    # Player death
    # --------------------------------------------------
    def die(self, pos):
        self.reset(pos)
        config.GAME_ACTIVE = False

    # --------------------------------------------------
    # Reset player to initial state
    # --------------------------------------------------
    def reset(self, pos):
        self.rect.midbottom = pos
        self.gravity = 0

        self.health.hp = self.health.max_hp
        self.invulnerable = False

        self.state = STATE_IDLE
        self.frame = 0

        self.attack_stage = 0
        self.combo_queued = False
        self.attacking = False

        self.moves = False
        self.jumped = False
